...
# For Websocket Port
var websocket_port_address = "9055"
var state = ""
...
#		socket.send("Hello!".to_utf8_buffer())
		while socket.get_available_packet_count():
			...
			if socket_data.get_string_from_utf8() == "updated":
				...
				one_frame = socket_data.get_string_from_utf8()
				one_frame = one_frame.replace("'", "\"")
				var test_json_conv = JSON.new()
				test_json_conv.parse(one_frame)
				one_frame = test_json_conv.get_data()
#				print("one frame: ", one_frame)
